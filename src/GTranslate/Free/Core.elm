module GTranslate.Free.Core exposing (..)
...
translate identifier config sourceText  =
    Http.send (getMessage config) <|
        Http.get ("https://translate.googleapis.com/translate_a/single?client=gtx&sl=" 
            ++ (sourceOrAuto config) 
            ++ "&tl=" ++ (getTargetLang config) 
            ++ "&dt=t&q=" ++ (Http.encodeUri sourceText)) 
        ...
        Just createUID -> 
            ...
            records 
                |> List.map(\ item -> 
                    ...
                    translations 
                        |> find (\t -> (id t) == Just (createUID item) )   -- find all elements that have this unique id
                        |> Maybe.andThen(\match -> Just (item |> update match) ) -- update the item
                        |> Maybe.withDefault item  -- If no matching element can be found, then just return the original item 
                ) 
...
batchStrings config lines =
    let
        ...
        source = sourceOrAuto config
        target = getTargetLang config

        commands = 
            case getGenerator config of 
                Just generator ->           -- if uidGenerator is present, use this instead
                    lines 
                        |> List.map(\item -> 
                            item |> translate (Just (generator  item)) config  
                        )
                Nothing ->
                    lines                    --- default to using the index as the id
    ...
    objects 
        |> List.indexedMap(\ index item -> 
            ...
            let 
                identifier =
                    case getGenerator config of 
                        Just func -> Just (func item)
                        Nothing -> Just (toString index)
            in 
...
zipMap strategy bothLists =
    ...
    let 
        ...
        (l1, l2) = helper bothLists ([],[])

        -- reverse both lists so that they are back in their original order
        orderedLists = (List.reverse l1, List.reverse l2)
    in
