module GTranslate.Free.Translation exposing (..)
...
decodeIndex index decoder arr =
    case (get index arr) of -- get object at index in aray
        Just v -> 
...
sentenceDecoder sentences = 
    sentences 
        |> Array.toList
        |> List.filterMap(\ sentence -> 

            let 
                -- Extract all values from the summary array
                decodedTranslation  = decodeIndex 0 string sentence
                decodedText         = decodeIndex 1 string sentence
            in 

             -- Ensure those summary values all exist and were decoded successfully
            case (decodedTranslation, decodedText ) of 
                (Just translatedText, Just originalText ) -> Just ({original = originalText, translated = translatedText})
                _ -> Nothing 
...
translationDecoder identifier targetLanguage =
  array value 
    |> andThen (\arr ->  

        let 
            -- extract variables
            decodedNestedArray  = decodeIndex 0 (array (array value)) arr 
            decodedLanguage     = decodeIndex 2 string arr 
            decodedConfidence   = decodeIndex 6 (maybe float) arr
        in 

        -- ensure that all values were decoded correctly
        case ( decodedNestedArray, decodedLanguage ) of 
            (Just sentences, Just sourceLanguage ) -> 

                let 
                    decodedSentences = (sentenceDecoder sentences)      
                    translated = decodedSentences |> List.map(\ e -> e.translated ) |>  List.foldl (++) ""
                    original = decodedSentences |> List.map(\ e -> e.original ) |> List.foldl (++) ""
                    conf =  (Maybe.withDefault Nothing decodedConfidence)
                in 

                succeed (Translation (TRecord translated original sourceLanguage targetLanguage conf identifier))

            ( Nothing, _ ) -> fail ("Missing translation object: " ++ toString decodedNestedArray)

            (_, Nothing ) -> fail ("Missing Language: " ++ toString decodedLanguage )
